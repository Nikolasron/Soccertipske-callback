<?php
// Load Paystack secret key from config.php (SAME FOLDER)
...
if ($package <= 0 || !$ref) {
    header("Location: https://soccertipske.wuaze.com/?error=invalid_callback");
    exit;
}
...
if ($result && isset($result['data']['status']) && $result['data']['status'] === 'success') {
    ...
    // Redirect back to your website with success
    header("Location: https://soccertipske.wuaze.com/?package=$package&verified=1#premium");
    exit;

} else {
    // Failed verification
    header("Location: https://soccertipske.wuaze.com/?package=$package&verified=0#premium");
    exit;
}
