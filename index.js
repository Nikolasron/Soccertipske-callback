...
app.post('/initiate-payment', async (req, res) => {
    ...
    console.log('API URL:', `${PAYNECTAR_API_URL}/payments/initiate`);
    console.log('Phone:', formattedPhone);
    console.log('Amount:', amount);
    console.log('Reference:', reference);
    console.log('API Key (first 10):', PAYNECTAR_SECRET_KEY.substring(0, 10) + '...');
    ...
    try {
        const response = await axios.post(`${PAYNECTAR_API_URL}/payments/initiate`, requestData, {
            headers: {
        ...
        console.log('✅ Paynectar Response Status:', response.status);
        console.log('✅ Paynectar Response Data:', JSON.stringify(response.data, null, 2));
        ...
        res.status(500).json({
            status: 'error',
            message: error.response?.data?.message || error.response?.data?.error || 'Failed to initiate payment. Please try again.',
            reference: reference,
